...
os.environ["TRANSFORMERS_CACHE"] = "/n/fs/nlp-df22/.cache/hf/"
os.environ["DATASETS_CACHE"] = "/n/fs/nlp-df22/.cache/hf/"
os.environ["HF_HOME"] = "/n/fs/nlp-df22/.cache/hf/"
...
def line(tensor, renderer=None, xaxis="", yaxis="", **kwargs):
    px.line(y=utils.to_numpy(tensor), labels={"x": xaxis, "y": yaxis}, **kwargs).show(
        renderer
    )
...
SAVE_DIR = Path("/n/fs/nlp-df22/project/attention-output-saes/checkpoints")
try:
    ...
    def refresh(self):
        ...
        with torch.autocast("cuda", torch.bfloat16):
            ...
            for _ in range(0, num_batches, self.cfg["model_batch_size"]):
                ...
                elif self.cfg["per_head"]:
                    acts = cache[self.cfg["act_name"]][:, :, self.cfg["head"]].reshape(
                        -1, self.cfg["act_size"]
                    )
                else:
...
# def get_batch_tokens_contiguous(dataset_iter, batch_size, model, cfg):
#     tokens = []
#     total_tokens = 0
#     seq_length = cfg["seq_len"]
#     while total_tokens < batch_size * seq_length:
#         try:
...
#         return None
#
#     reshaped_tokens = torch.stack(tokens, dim=0)
...
def get_batch_tokens_contiguous(dataset_iter, batch_size, model, cfg):
    ...
    total_batches = 0
    seq_length = cfg["seq_len"]
    while total_batches < batch_size:
        try:
            # Retrieve next item from iterator
            row = next(dataset_iter)["text"]
        except StopIteration:
        ...
        cur_toks = cur_toks[: batch_size - total_batches]
        tokens.append(cur_toks)
        total_batches += cur_toks.shape[0]
...
def get_batch_tokens(dataset_iter, batch_size, model, cfg):
    if cfg.get("contiguous", False):
        return get_batch_tokens_contiguous(dataset_iter, batch_size, model, cfg)
    tokens = []
    total_tokens = 0
...
def per_head_zero_ablate_hook(z, hook, head):
    # z_head = einops.rearrange(z[:, :, head], "batch pos d_head -> (batch pos) d_head")
    # z_head[:] = 0.0
    # z_head = einops.rearrange(
    #     z_head,
    #     "(batch pos) d_head -> batch pos d_head",
    #     batch=z.shape[0],
    #     pos=z.shape[1],
    # ).unsqueeze(2)
    z_zero = torch.zeros_like(z[:, :, head : head + 1], device=z.device)
    z_reconstr = torch.cat([z[:, :, :head], z_zero, z[:, :, head + 1 :]], dim=2)
    return z_reconstr
...
def replace_per_head_z_hook(z, hook, head, encoder):
    z_head = einops.rearrange(z[:, :, head], "batch pos d_head -> (batch pos) d_head")
    z_reconstr_head = einops.rearrange(
        encoder(z_head)[1],
        "(batch pos) d_head -> batch pos d_head",
        batch=z.shape[0],
        pos=z.shape[1],
    ).unsqueeze(2)
    z_reconstr = torch.cat(
        [z[:, :, :head], z_reconstr_head, z[:, :, head + 1 :]], dim=2
    )
    return z_reconstr
...
@torch.no_grad()
def get_recons_loss(
    model, all_tokens, num_batches, cfg, local_encoder=None, dataset_iter=None
):
    ...
    for i in range(num_batches):
        if dataset_iter is not None:
            ...
            tokens = all_tokens[
                torch.randperm(len(all_tokens))[: cfg["model_batch_size"]]
            ]
        ...
        if cfg["concat_heads"]:
            recons_loss = model.run_with_hooks(
                ...
                fwd_hooks=[
                    (
                        cfg["act_name"],
                        partial(replace_concat_z_hook, encoder=local_encoder),
                    )
                ],
            )
        elif cfg["per_head"]:
        ...
        if cfg["per_head"]:
            zero_abl_loss = model.run_with_hooks(
                ...
                fwd_hooks=[
                    (
                        cfg["act_name"],
                        partial(per_head_zero_ablate_hook, head=cfg["head"]),
                    )
                ],
            )
        else:
...
def get_freqs(
    ...
    for i in tqdm.trange(num_batches, disable=disable_tqdm):
        ...
        if cfg["concat_heads"]:
            ...
            acts = acts[:, :, cfg["head"]].reshape(-1, cfg["act_size"])
        else:
...
def re_init(indices, encoder):
    ...
    encoder.W_dec.data[indices, :] /= torch.norm(
        encoder.W_dec.data[indices, :], dim=1, keepdim=True
    )
...
def anthropic_resample(buffer, indices, encoder, verbose=False):
    ...
    alive_neurons = torch.arange(encoder.d_hidden, device=encoder.device)[
        ~indices
    ]  # [n_alive,]
    dead_features = torch.arange(encoder.d_hidden, device=encoder.device)[
        indices
    ]  # [n_dead,]
    ...
    global_losses = torch.zeros(
        (encoder.cfg["anthropic_resample_batches"] * encoder.cfg["batch_size"],),
        dtype=encoder.dtype,
        device=encoder.device,
    )
    ...
    pbar = (
        tqdm.tqdm(range(encoder.cfg["anthropic_resample_batches"]))
        if verbose
        else range(encoder.cfg["anthropic_resample_batches"])
    )
    for i in pbar:
        ...
        global_losses[
            i * encoder.cfg["batch_size"] : (i + 1) * encoder.cfg["batch_size"]
        ] = l2_loss
    ...
    replacement_indices = torch.multinomial(
        distn.probs, num_samples=n_dead, replacement=False
    )  # sample without replacement ig?

    # Index into the batch of hidden activations to get our replacement values
    replacement_values = (global_input_activations - encoder.b_dec)[
        replacement_indices
    ...
    W_enc_norm_alive_mean = (
        ...
        else encoder.W_enc[:, alive_neurons].norm(dim=0).mean().item()
    )
    ...
    encoder.W_enc.data[:, dead_features] = (
        replacement_values_normalized.T
        * W_enc_norm_alive_mean
        * encoder.cfg["anthropic_neuron_resample_scale"]
    )
...
def get_encoder_hidden_acts(tokens, encoder, model, feature_id=None):
    _, cache = model.run_with_cache(
        tokens, return_type=None, names_filter=encoder.cfg["act_name"]
    )
    acts = cache[encoder.cfg["act_name"]]
    if encoder.cfg["concat_heads"]:
        acts = einops.rearrange(
            acts, "batch pos n_heads d_head -> (batch pos) (n_heads d_head)"
        )
    elif encoder.cfg["per_head"]:
        acts = acts[:, :, encoder.cfg["head"]].reshape(-1, encoder.cfg["act_size"])
    else:
...
def get_per_src_pos_dfa_all_heads(tokens, feature_id, encoder, model):
    ...
    )  # [batch, dest_pos, src_pos, n_heads*d_head]

    # Element-wise multiplication, removing the loop
    decomposed_z_cat = attn_weights_bcast * v_cat.unsqueeze(
        1
    )  # [batch, dest_pos, src_pos n_heads*d_head]

    per_src_pos_dfa = einops.einsum(
        decomposed_z_cat,
        encoder.W_enc[:, feature_id],
...
def replace_with_recons_acts_hook(
    act, hook, encoder, ablated_feature=None, pinned_feature=None, pinned_value=None
):
    # Decode activation into features
    if encoder.cfg["concat_heads"]:
        assert act.ndim == 4
        reshaped_acts = einops.rearrange(
            act, "batch pos n_heads d_head -> (batch pos) (n_heads d_head)"
        )
    elif encoder.cfg["per_head"]:
        raise NotImplementedError(
            "replace_with_recons_act_hook not implemented for per_head SAE"
        )
    else:
