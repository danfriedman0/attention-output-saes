...
class AutoEncoder(nn.Module):
    ...
    def get_checkpoint_name(self):
        name = (
            f"{self.cfg['model_name']}_L{self.cfg['layer']}_H{self.cfg['head']}_{self.cfg['site']}"
            f"_lr{self.cfg['lr']:.2e}_l1{self.cfg['l1_coeff']:.2e}_ds{self.cfg['dict_size']}"
            f"_nt{self.cfg['num_tokens']}"
            f"_bs{self.cfg['batch_size']}_dc{self.cfg['dead_direction_cutoff']:.2e}"
            f"_rs{self.cfg['resample_scheme']}_rie{self.cfg['re_init_every']}_nr{self.cfg['num_resamples']}"
            f"_sl{self.cfg['seq_len']}_c{int(self.cfg['contiguous'])}"
        )
    ...
    @classmethod
    def load_from_hf(
        cls, version, hf_repo="ckkissane/tinystories-1M-SAES", verbose=False
    ):
        """
        ...
        self.load_state_dict(
            download_file_from_hf(hf_repo, f"{version}.pt", force_is_torch=True)
        )
        return self

    @torch.no_grad()
    def initialize_b_dec_with_geometric_median(
        self, activation_store, num_batches=50, maxiter=100_000
    ):
